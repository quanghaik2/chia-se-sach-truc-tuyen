...
const getAllUsers = () =>
   new Promise(async (resolve, reject) => {
      const data = await models.User.find({}).select('-password -createdAt -updatedAt -isDeleted -__v').where({ isDeleted: false });
      resolve({
         data: data ? data : null,
      });
   });
...
const getOneUser = (userId) =>
   new Promise(async (resolve, reject) => {
      const data = await userRepositories.findByIdUser(userId).select('-password -createdAt -updatedAt -isDeleted -__v').where({ isDeleted: false });
      resolve({
         err: data ? false : true,
...
const getUserByName = (name) =>
   new Promise(async (resolve, reject) => {
      const data = await models.User.find({ name }).select('-password -createdAt -updatedAt -isDeleted -__v').where({ isDeleted: false });
      resolve({
         err: data ? false : true,
         message: data ? 'Get user successfully' : 'User not found',
         data: data ? data : null,
