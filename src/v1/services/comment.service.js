...
const createComment = ({ bookId, ...body }, userId) =>
   new Promise(async (resolve, reject) => {
      ...
      if (data) {
         await models.Book.findByIdAndUpdate(data.bookId, {
            $push: { commentList: data.id },
         });
         await models.User.findByIdAndUpdate(data.userId, {
            $push: { commentedList: data.id },
         });
      }
...
const updateComment = ({ commentId, ...body }, userId) =>
   new Promise(async (resolve, reject) => {
      // const book = await models.Book.findById(bookId);
      // if(!book){
      //     resolve({
      ...
      //     })
      // }
      const checkComment = await models.Comment.find({
         id: commentId,
         userId: userId,
      });
      if (!checkComment) {
      ...
      const data = await models.Comment.findByIdAndUpdate(
         commentId,
         { ...body },
         { new: true }
      );
...
const softDeleteComment = ({ commentId }, userId) =>
   new Promise(async (resolve, reject) => {
      const checkComment = await models.Comment.find({
         id: commentId,
         userId: userId,
